...
#include <sstream>
RadioList::RadioList(const std::string title, uint8_t& selectedId, std::initializer_list<std::string> list):
		MenuItem(title),
		items(list),
		activeItemRef(selectedId){
	activeItem = secureId(activeItemRef);
}

RadioList::RadioList(const std::string title, std::function<void(RadioList*)> callback, std::initializer_list<std::string> list) :
		MenuItem(title),
		items(list),
		activeItem(0),
