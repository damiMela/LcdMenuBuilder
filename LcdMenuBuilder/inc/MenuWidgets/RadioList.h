...
{
public:
	RadioList(const std::string titile, uint8_t &selectedId, std::vector<std::string> list);
	RadioList(const std::string titile, std::function<void(RadioList *)> callback, std::vector<std::string> list);
	virtual ~RadioList();
...
private:
	...
	uint8_t *activeItemFeedback = nullptr;
